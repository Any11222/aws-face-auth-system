...
function RegisterPage() {
  ...
  const registerEmployee = async () => {
    if (!livenessPassed || !capturedImage) {
      setResult("Please complete AWS Face Liveness first.");
      return;
    }
    ...
    if (
      ...
      setResult("Please fill all employee details.");
      return;
    }
    ...
    <div className="page-container">
      <div className="split-card">
        ...
        <div className="right-panel">
          ...
          <div className="live-status">{livenessMsg}</div>
          ...
          <div className="status-box">{result}</div>
        </div>
      </div>
    </div>
  );
